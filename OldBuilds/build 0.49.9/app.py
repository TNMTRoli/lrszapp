...
def calc():
    ...
    if '' not in (values['-INPUT_H_START-'], values['-INPUT_M_START-'], values['-INPUT_H_END-'], values['-INPUT_M_END-']): 
        ...
        # Ha van benne üres string, akkor nem jó.
    #elif '' in (values['-INPUT_H_START-'], values['-INPUT_M_START-'], values['-INPUT_H_END-'], values['-INPUT_M_END-']):
    #    mw['-RESULT_TEXT-'].update(visible=False)
    #    mw['-ERROR_LOG-'].update(visible=True)
    #    mw['-ERROR_LOG-'].update('Log: Kérlek, írj be egy számot az összes helyre.')
...
while True:
    ...
    # -------- ha megnyomjuk a calculate gombot, akkor lefut.
    #if event == 'calculate':
    #    calc()

    #if event == '-INPUT_H_START-' or '-INPUT_M_START-' or '-INPUT_H_END-' or '-INPUT_M_END-':
       # if values['-INPUT_H_START-'] and values['-INPUT_M_START-'] and values['-INPUT_H_END-'] and values['-INPUT_M_END-'] is not None:
    calc()
